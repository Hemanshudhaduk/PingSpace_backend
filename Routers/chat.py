...
def delete_server(server_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    db.query(ServerUser).filter(ServerUser.server_id == server_id).delete(synchronize_session=False)
    
    room_ids = [r[0] for r in db.query(Room.id).filter(Room.server_id == server_id).all()]
    if room_ids:
...
def create_server_user(payload: ServerUserCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    is_admin = admin_check and (server.admin_id == current_user.id or admin_check.role == "admin")
    
    if not (is_self_join or is_admin):
        raise HTTPException(403, "Not allowed: Only an admin can add other users, or you can join as a member yourself")
