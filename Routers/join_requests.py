...
from schemas.join_request_schema import (
    JoinRequestCreate,
    JoinRequestOut,
    JoinRequestUpdate,
)
...
def create_join_request(
    ...
    existing = (
        db.query(ServerUser)
        .filter(
            ServerUser.server_id == request.server_id,
    ...
    pending = (
        ...
        .filter(
            JoinRequest.server_id == request.server_id,
...
def get_join_requests(
    ...
    requests = (
        db.query(JoinRequest)
        .filter(JoinRequest.server_id == server_id, JoinRequest.status == "pending")
        .all()
    )
    return requests
